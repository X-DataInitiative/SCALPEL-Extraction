...
class ColumnUtilitiesSuite extends SharedContext {
  ...
  "parseTimestamp" should "convert a String column to a Timestamp column" in {
    ...
    val result = input.select(
      ColumnUtilities.parseTimestamp(col("string_col"), pattern).as("timestamp_col")
    )
  ...
  it should "throw an exception if a string is unparseable" in {
    ...
    intercept[org.apache.spark.SparkException] {
      input.select(ColumnUtilities.parseTimestamp(col("string_col"), pattern)).show
    }
  }
  ...
  "parseDate" should "convert a String column to a Date column" in {
    ...
    // When
    val result = input.select(ColumnUtilities.parseDate(col("string_col"), pattern).as("date_col"))
  ...
  it should "throw an exception if a string is unparseable" in {
    ...
    // WhenThen
    intercept[org.apache.spark.SparkException] {
      input.select(ColumnUtilities.parseDate(col("string_col"), pattern)).show
    }
  }
