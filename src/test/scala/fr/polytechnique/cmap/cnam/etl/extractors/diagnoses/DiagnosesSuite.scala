...
class DiagnosesSuite extends SharedContext {
  ...
  "extract" should "call the adequate private extractors" in {
    ...
    // Given
    val config = DiagnosesConfig(dpCodes = List("C67", "C77", "C78", "C79"), drCodes = List("C67", "C77", "C78", "C79"), daCodes = List("C67"), imbCodes = List("C67"))
    val mco: DataFrame = McoPreProcessor.prepareDF(sqlContext.read.load("src/test/resources/test-input/MCO.parquet"))
