...
class MedicalTakeOverReasonSuite extends FlatSpec {
  ...
  "fromRow" should "allow creation of a MedicalTakeOverReason Builder event from a row object" in {
    ...
    val values = Array[Any]("Patient01", "1_1_2010", "11", makeTS(2010, 1, 1))
    val r = new GenericRowWithSchema(values, schema)
    val expected = MockMedicalTakeOverReason("Patient01", "1_1_2010", "11", makeTS(2010, 1, 1))

    // When
    val result = MockMedicalTakeOverReason.fromRow(r, "pID", "gId", "cod", "dat")
