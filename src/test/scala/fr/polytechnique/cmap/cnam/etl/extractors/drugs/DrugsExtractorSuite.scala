...
class DrugsExtractorSuite extends SharedContext{
  ...
  "extract" should "work correctly based on the DrugConfig Antidepresseurs" in {
    ...
    // When
    val result = DrugsExtractor.extract(TherapeuticLevel, source, List(drugConfig))
  ...
  "extract" should "return expected drug purchases with Therapeutic level" in {
    ...
    // When
    val result = DrugsExtractor.extract(TherapeuticLevel, source, List(drugConfigAntidepresseurs, drugConfigNeuroleptiques))
  ...
  "extract" should "return expected drug purchases with Pharmacological level" in {
    ...
    val drugConfigNeuroleptiques: DrugConfig = Neuroleptiques
    // When
    val result = DrugsExtractor.extract(PharmacologicalLevel, source, List(drugConfigAntidepresseurs, drugConfigNeuroleptiques))
  ...
  "extract" should "return expected drug purchases with Molecule level" in {
    ...
    val source = new Sources(irPha = Some(Seq(
      ...
      (Some("3400935183644"), "N06AA04", ""),
      (Some("3400935418487"), "A10BB09", ""),
      (Some("3400936889651"), "N06AB03", "DEXTROPROPOXYPHENE_PARACETAMOL_CAFEINE"),
      (Some("3400930023648"), "N05AX12", "INSULINE LISPRO (PROTAMINE)")
    ).toDF("PHA_CIP_C13", "PHA_ATC_C07", "molecule_combination")
    ), dcir = Some(inputDF))
    ...
    val result = DrugsExtractor
      .extract(MoleculeLevel, source, List(drugConfigAntidepresseurs, drugConfigNeuroleptiques))
  ...
  "extract" should "return expected drug purchases with MoleculeCombination level" in {
    ...
    val expected: Dataset[Event[Drug]] = Seq(
      Drug("patient2", "", 0.0, makeTS(2014, 6, 1)),
      Drug("patient4", "", 0.0, makeTS(2014, 8, 1)),
      Drug("patient8", "DEXTROPROPOXYPHENE_PARACETAMOL_CAFEINE", 0.0, makeTS(2014, 9, 1)),
      Drug("patient2", "INSULINE LISPRO (PROTAMINE)", 0.0, makeTS(2014, 6, 1))
    ).toDS
    ...
    // When
    val result = DrugsExtractor
      .extract(MoleculeCombinationLevel, source, List(drugConfigAntidepresseurs, drugConfigNeuroleptiques))
