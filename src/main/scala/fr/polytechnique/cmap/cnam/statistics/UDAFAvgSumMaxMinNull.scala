...
class UDAFAvgSumMaxMinNull()
  ...
  override def bufferSchema: StructType = {
    new StructType().add("sumInter", LongType)
      .add("countInter", LongType)
      .add("maxInter", LongType)
      .add("minInter", LongType)
      .add("NULLInter", LongType)
  }
  ...
  override def update(buffer: MutableAggregationBuffer, input: Row): Unit = {
    if (!input.isNullAt(0)) {
      ...
      buffer(2) = {
        if (buffer(1) == 1L) inputColValue
        else scala.math.max(inputColValue, buffer.getLong(2))
      }

      buffer(3) = {
        if (buffer(1) == 1L) inputColValue
        else scala.math.min(inputColValue, buffer.getLong(3))
      }
    }
    else
  ...
  override def merge(buffer1: MutableAggregationBuffer, buffer2: Row): Unit = {
    ...
    buffer1(2) = {
      if (buffer1.getLong(1) != 0L && buffer2.getLong(1) != 0L)
        scala.math.max(buffer1.getLong(2), buffer2.getLong(2))
      else {
        if (buffer1.getLong(1) == 0L) buffer2.getLong(2)
          else buffer1.getLong(2)
      }
    }

    buffer1(3) = {
      if (!(buffer1.getLong(1) == 0L || buffer2.getLong(1) == 0L))
        scala.math.min(buffer1.getLong(3), buffer2.getLong(3))
      else {
        if (buffer1.getLong(1) == 0L) buffer2.getLong(3)
          else buffer1.getLong(3)
      }
    }
