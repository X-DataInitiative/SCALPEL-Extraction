...
class DcirSourceExtractor(
  ...
  override val extractors = List(
    ExtractorSources[Drug, DrugConfig](new DrugExtractor(drugConfig), List("ER_PRS_F", "ER_PHA_F", "IR_PHA_R"), "DRUG_PURCHASES"),
    ExtractorSources[MedicalAct, SimpleExtractorCodes](DcirMedicalActExtractor(SimpleExtractorCodes.empty), List("ER_PRS_F", "ER_CAM_F", "ER_ETE_F"), "DCIR_MEDICAL_ACT"),
