...
import fr.polytechnique.cmap.cnam.etl.extractors.events.drugs.classification.families.Opioids
...
object OpioidsExtractor {
  def extract(sources: Sources): Dataset[Event[Drug]] = {
    new DrugExtractor(DrugConfig(TherapeuticLevel, List(Opioids))).extract(sources)
  }
}
