...
object MLPPConfig {
  ...
  val bucketSizes: List[Int] = conf.getIntList("bucket_size").asScala.toList.map(_.toInt)
  val lagCounts: List[Int] = conf.getIntList("lag_count").asScala.toList.map(_.toInt)
  val minTimestamp: Timestamp = makeTS(conf.getIntList("min_timestamp").asScala.toList)
  val maxTimestamp: Timestamp = makeTS(conf.getIntList("max_timestamp").asScala.toList)
  val includeCensoredBucket: Boolean = conf.getBoolean("include_censored_bucket")
  val featuresAsList: Boolean = conf.getBoolean("features_as_list")
