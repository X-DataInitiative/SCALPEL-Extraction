...
trait BasicExtractor[EventType <: AnyEvent] extends Extractor[EventType, BaseExtractorCodes] {
  ...
  def extractValue(row: Row): String = row.getAs[String](columnName)
}

sealed trait InStudyStrategy[EventType <: AnyEvent] {
  self: BasicExtractor[EventType]=>
